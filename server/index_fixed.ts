...
app.get('/proveedores', async (req, res) => {
    const proveedores = await prisma.proveedor.findMany({ include: { productos: true } });
...
app.get('/productos', async (req, res) => {
    const productos = await prisma.producto.findMany({ include: { proveedor: true, categoria: true } });
    res.json(productos);
});
...
app.post('/caja', async (req, res) => {
    const caja = await prisma.caja.create({ data: req.body });
    res.json(caja);
});
