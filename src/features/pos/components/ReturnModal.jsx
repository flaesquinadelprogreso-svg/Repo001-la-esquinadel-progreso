        ...
        <Modal isOpen={true} onClose={onClose} title={`Devolución - Recibo ${selectedVenta.numeroRecibo}`} size="lg">
            <div style={{ minWidth: '600px', maxWidth: '800px' }}>
                ...
                <div style={{ marginBottom: '16px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                        ...
                        <button
                            ...
                            style={{ padding: '4px 12px', fontSize: '12px', border: '1px solid #ECECEC', borderRadius: '4px', backgroundColor: '#FFF', cursor: 'pointer', fontWeight: 500 }}
                        >
                            Devolver Todo
                        </button>
                    </div>
                    <div style={{ border: '1px solid #ECECEC', borderRadius: '8px', overflow: 'hidden' }}>
                        <div style={{ maxHeight: '300px', overflow: 'auto' }}>
                            <table style={{ width: '100%', fontSize: '13px', borderCollapse: 'collapse' }}>
                                <thead>
                                    <tr style={{ backgroundColor: '#F9FAFB', borderBottom: '1px solid #ECECEC', position: 'sticky', top: 0, zIndex: 1 }}>
                                            ...
                                            <tr key={item.id} style={{ borderBottom: '1px solid #F3F4F6' }}>
                                                <td style={{ padding: '10px' }}>
                                                    ...
                                                    <div style={{ fontSize: '11px', color: '#9CA3AF' }}>
                                                        ...
                                                        {item.esServicio && <span style={{ marginLeft: '8px', color: '#D69A00' }}>Servicio</span>}
                                                    </div>
                                                </td>
                                                    ...
                                                    {disponible > 0 ? (
                                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}>
                                                            <button
                                                                ...
                                                                style={{ padding: '2px 8px', border: '1px solid #ECECEC', borderRadius: '4px', backgroundColor: '#F3F4F6', cursor: 'pointer' }}
                                                            >
                                                                -
                                                            </button>
                                                            <input
                ...
                {Object.values(returnItems).some(qty => qty > 0) && (
                    <div style={{ marginBottom: '16px', padding: '12px', backgroundColor: '#EFF4FF', borderRadius: '12px', border: '1px solid rgba(37,99,235,0.2)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                            <span style={{ fontSize: '13px', color: '#2563EB' }}>Subtotal a devolver:</span>
                            <span style={{ fontWeight: 600 }}>{formatPesos(calculateReturnTotals().subtotal)}</span>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                            <span style={{ fontSize: '13px', color: '#2563EB' }}>IVA:</span>
                            <span style={{ fontWeight: 600 }}>{formatPesos(calculateReturnTotals().iva)}</span>
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 700, fontSize: '16px', borderTop: '1px solid rgba(37,99,235,0.2)', paddingTop: '8px', marginTop: '8px' }}>
                        ...
                        {[
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    padding: '10px',
                                    borderRadius: '8px',
                                    border: returnMethod === method.id ? '2px solid #F5B400' : '1px solid #ECECEC',
                                    backgroundColor: returnMethod === method.id ? '#FFF9E6' : '#fff',
                                    cursor: 'pointer',
                                    display: 'flex',
                            ...
                            >
                                <method.icon size={18} color={returnMethod === method.id ? '#D69A00' : '#6B7280'} />
                                <span style={{ fontSize: '12px', fontWeight: 600, color: returnMethod === method.id ? '#D69A00' : '#6B7280' }}>
                                    {method.label}
                                </span>
                            </button>
                        ))}
                ...
                {Object.entries(returnItems).some(([id, qty]) => {
                    ...
                    <div style={{ marginBottom: '16px', padding: '10px', backgroundColor: '#FFF9E6', borderRadius: '12px', display: 'flex', alignItems: 'flex-start', gap: '8px' }}>
                        <AlertCircle size={18} color="#D69A00" style={{ flexShrink: 0, marginTop: '2px' }} />
                        <div style={{ fontSize: '12px', color: '#D69A00' }}>
                ...
                {/* Actions */}
                <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', borderTop: '1px solid #ECECEC', paddingTop: '16px' }}>
