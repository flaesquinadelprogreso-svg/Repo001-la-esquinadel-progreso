...
export default function ConfiguracionCajas() {
    ...
    if (view === 'create' || view === 'edit') {
        return (
            <div id="cajas-form" style={{ display: 'flex', flexDirection: 'column', gap: '24px', maxWidth: '800px' }}>
                ...
                <div style={{ backgroundColor: '#fff', border: '1px solid #ECECEC', borderRadius: '10px', padding: '24px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
                    {/* Datos Generales */}
                    <div>
                        <h3 style={{ fontSize: '15px', fontWeight: 600, color: '#111827', marginBottom: '16px', borderBottom: '1px solid #ECECEC', paddingBottom: '8px' }}>1. Datos generales de tu caja</h3>
                        ...
                        <div style={{ marginTop: '12px', padding: '10px', backgroundColor: '#FDECEC', border: '1px solid #DC2626', borderRadius: '6px', display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                            <AlertCircle size={16} color="#DC2626" style={{ marginTop: '2px', flexShrink: 0 }} />
                            <span style={{ fontSize: '12px', color: '#DC2626' }}><strong>Importante:</strong> Es necesario asociar usuarios a la caja, caso contrario el sistema generará un error de ingreso.</span>
                        </div>
                    </div>

                    {/* Precios y Descuentos */}
                    <div>
                        <h3 style={{ fontSize: '15px', fontWeight: 600, color: '#111827', marginBottom: '16px', borderBottom: '1px solid #ECECEC', paddingBottom: '8px' }}>2. Precios y Descuentos</h3>
                        ...
                        <p style={{ fontSize: '13px', fontWeight: 600, color: '#6B7280', marginTop: '16px', marginBottom: '8px' }}>Listas de Precio Habilitadas</p>
                ...
                <Modal isOpen={showCancelModal} onClose={() => setShowCancelModal(false)} title="¿Descartar cambios?">
                    <p style={{ fontSize: '14px', color: '#6B7280', marginBottom: '20px' }}>Si sales ahora, perderás todos los datos ingresados. ¿Estás seguro de cancelar la creación/edición?</p>
    ...
    return (
        <div id="cajas-config-root" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <div id="cajas-config-table" style={{ backgroundColor: '#fff', border: '1px solid #ECECEC', borderRadius: '10px', overflow: 'hidden' }}>
                    ...
                    <tbody>
                        {cajas.map(c => (
                            <tr key={c.id} style={{ borderBottom: '1px solid #F8F9FB' }}>
                                <td style={{ padding: '14px 20px', fontSize: '14px', fontWeight: 600, color: '#111827' }}>{c.name}</td>
                                <td style={{ padding: '14px 20px', fontSize: '13px', color: '#6B7280' }}>{c.sucursal}</td>
                                ...
                                <td style={{ padding: '14px 20px', textAlign: 'right' }}>
                                    <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                                        <button onClick={() => initForm(c)} disabled={c.status === 'abierta'} style={{ padding: '6px', borderRadius: '6px', border: '1px solid #ECECEC', background: '#FFFFFF', cursor: c.status === 'abierta' ? 'not-allowed' : 'pointer', opacity: c.status === 'abierta' ? 0.5 : 1 }}>
                                            <Edit size={14} style={{ color: '#6B7280' }} />
                                        </button>
                                        <button style={{ padding: '6px', borderRadius: '6px', border: '1px solid #ECECEC', background: '#FFFFFF', cursor: 'pointer' }}>
                                            <ShieldBan size={14} style={{ color: '#DC2626' }} />
                                        </button>
                                    </div>
